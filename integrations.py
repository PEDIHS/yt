...
def resolve_instagram_cookie_blob() -> str:
...
def instagram_status() -> dict:
    ...
    return {
        "configured": bool(sessionid or cookie_blob),
        "username": get_secret("instagram_username"),
        "user_id": get_secret("instagram_user_id"),
        "cookie_mode": bool(cookie_blob),
    }


def build_instagram_cookie_blob(sessionid: str, csrftoken: str = "", ds_user_id: str = "") -> str:
    ...
    rows = [
        ...
        "# Generated by YT Studio",
        f".instagram.com\tTRUE\t/\tTRUE\t0\tsessionid\t{sessionid}",
    ]
    if csrftoken:
        rows.append(f".instagram.com\tTRUE\t/\tTRUE\t0\tcsrftoken\t{csrftoken}")
    if ds_user_id:
        rows.append(f".instagram.com\tTRUE\t/\tTRUE\t0\tds_user_id\t{ds_user_id}")
    return "\n".join(rows) + "\n"
